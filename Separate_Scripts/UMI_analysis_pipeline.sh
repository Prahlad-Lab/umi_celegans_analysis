...
DIR_VCF_INPUT="/vscratch/grp-vprahlad/umi_analysis/data/input"
...
PYTHON_SCRIPT_PATH="/projects/rpci/vprahlad/umi_celegans/analysis_broad/calculate_allele_proportions_depth10.py"
...
parallel --xapply FastqToSam \
      SORT_ORDER=unsorted \
      F1=${DIR_SEQS}/{}_R1.fastq.gz \
      F2=${DIR_SEQS}/{}_R2.fastq.gz \
      SM={} \
      LB="L1" \
      PL="ILLUMINA" \
      PU="barcode1" \
...
conda activate fgbio # As specified in the original STAR script
...
#==============================================================================
# STEP 11: FilterBambyFamilySize (Create BAMs for singletons and families 2-10)
#==============================================================================
...
# --- Part B: Process Families 2-10 (Family Size >= 2 and <= 10) ---
echo "--- Step 11d: Extracting Family 2-10 IDs ---"
for i in "${sample[@]}"
do
    echo "Processing families 2-10 for sample: $i"
	awk 'BEGIN{FS="\t"} NR > 1 && $8 >= 2 && $8 <= 10 {print $9}' "${DIR_UMI_MARKING}/${i}.grouped.tsv" | sort -u > "${DIR_FILTER_BAM}/${i}.families_2-10_id.txt"
done;

echo "--- Step 11e: Creating Family 2-10 BAMs ---"
for i in "${sample[@]}"
do
    echo "Filtering BAM for families 2-10: $i"
	samtools view -@ 8 -h --tag-file UG:"${DIR_FILTER_BAM}/${i}.families_2-10_id.txt" -o "${DIR_FILTER_BAM}/${i}.families_2-10.bam" "${DIR_UMI_MARKING}/${i}.grouped_by_UMI.bam"
done;

echo "--- Step 11f: Sorting and Indexing Family 2-10 BAMs ---"
for i in "${sample[@]}"
do
    echo "Sorting/Indexing families 2-10: $i"
	samtools sort -@ 8 -o "${DIR_FILTER_BAM}/${i}.sort_coood_families_2-10.bam" "${DIR_FILTER_BAM}/${i}.families_2-10.bam";
	samtools index -@ 8 "${DIR_FILTER_BAM}/${i}.sort_coood_families_2-10.bam"	
done;

echo "--- Finished Step 11: Filter BAMs by Family Size ---"

#==============================================================================
# STEP 12: GATK BQSR Pipeline for Singletons
#==============================================================================
...
#==============================================================================
# PATH A - STEP 13: Variant Calling (HaplotypeCaller) & Annotation (SnpEff)
#==============================================================================
...
module load gcc python
# Assuming no separate conda env is needed for the python script,
# or that 'python' is available in the 'bcftools' env or base env.
